# 🚀 Ramish's Career Switch — Master Repository
### TCS 8.29 LPA → BFSI GCC 14-18+ LPA | Active: September Muay Thai Camp

> **84 days. One mission. Sunnah path.**
> This duniya is temporary. Akhirah is permanent. Both get your best.

---

## 📊 Progress Dashboard

> All counts live in `session-state.md` — the SINGLE SOURCE OF TRUTH.
> If any file disagrees, `session-state.md` wins. See `tracker/state-machine.md`.
> **Daily schedule lives in `DAILY-BLUEPRINT.md` (September Camp edition).**
> **Reel shot list + editing workflow lives in Hermes skill `gemini-omni-reel`.**

| Metric | Status |
|--------|--------|
| **Active Phase** | September 1–30, 2026 — Muay Thai Camp at BRUTE Manpada |
| **Plan Version** | V3-restart (Sep 1 start, Sept camp active) |
| **Current Day** | Day 3 of 84 (Sept 3, 2026) |
| **DSA Problems Solved** | 0 (fresh start — carryover cleared) |
| **DSA Unaided** | 0/0 (fresh start) |
| **System Designs Practiced** | 0 / 8 |
| **Mock Interviews Done** | 0 / 12 |
| **Applications Sent** | 0 |
| **Plan A/B/C** | A = Java+Spring GCC (primary) |

---

## 🛤️ The Three Paths

| Plan | Path | Target | Trigger |
|------|------|--------|---------|
| **🅰️ Primary** | Java + Spring Boot → BFSI GCC | ₹16-50 LPA | Default |
| **🅱️ Fallback** | AI/GenAI Engineer → Product Co | ₹20-40 LPA | If Spring struggles + AI thriving by Week 6 |
| **🅲️ Safety Net** | Stay TCS, switch Q1 2027 | Same targets | If market crashes or no offers by Nov |

---

## 🎯 Target Companies (BFSI GCCs — all actively hiring in India)

| Company | India Headcount | Location |
|---------|----------------|----------|
| JPMorgan Chase | ~55,000 | Mumbai/Bangalore |
| HSBC | ~42,000 | Hyderabad/Bangalore |
| Wells Fargo | ~37,000 | Bangalore/Hyderabad |
| Barclays | ~20,000 | Pune |
| Deutsche Bank | ~13,000 | Pune/Bangalore |
| Standard Chartered | ~15-18,000 | Chennai/Bangalore |
| Goldman Sachs | — | Bangalore |
| Morgan Stanley | — | Mumbai/Bangalore |
| BNY Mellon | — | Pune |

**GCC roles are almost NEVER on Naukri.** Filled through LinkedIn, referrals, headhunters.

---

## 📁 Repo Structure (Clean — Sep 1, 2026)

```
career-switch-plan/
├── README.md                     ← You are here (dashboard)
├── MASTER-BLUEPRINT.md           ← Everything in one document
├── DAILY-BLUEPRINT.md            ← Daily routine — September Camp edition (print this, put on wall)
├── ramish-12-week-plan-v2.md     ← Full 12-week plan (week-by-week)
├── session-state.md              ← SINGLE SOURCE OF TRUTH (auto-updated by Hermes)
├── DIET-PLAN.md                   ← Competition cut diet (83→70kg)
│
├── [DELETED] REELS-GUIDE.md      ← Migrated to Hermes skill `gemini-omni-reel`
├── [DELETED] REELS-PLAN.md       ← Migrated to Hermes skill `gemini-omni-reel`
│
├── tracker/
│   ├── state-machine.md          ← System rules: edge cases, bar calc, scenarios
│   ├── progress.md               ← 84-day grid (V2 aligned, no duplicate counts)
│   ├── dsa-tracker.md            ← DSA problem log (per-problem, not aggregate)
│   ├── habit-tracker.md          ← Bullet journal system (prayers, sleep, reels, habits)
│   ├── weekly-review-template.md ← Friday review questions
│   └── job-application-tracker.md ← Applications, referrals, interviews
│
├── career/
│   ├── resume-v1-draft.md         ← Resume draft v1
│   └── resume-v2-updated.md       ← Resume v2 (from actual PDF, with gaps marked)
│
└── study-materials/
    ├── core-java-interview-bank.md
    ├── spring-boot-interview-notes.md
    ├── system-design-cheatsheet.md
    └── sql-interview-prep.md
```

**DELETED (before Sep 1, 2026):** ramish-12-week-plan.md (V1), ramish-week1-plan.md (V1), week1-routine.md (V1), system-state.md (ghost SOT — merged into state-machine), REELS-DAY-IN-LIFE.md, REELS-SHOOT-GUIDE.md, reels-guide.md (all merged into REELS-GUIDE.md)

---

## ⏰ Daily Structure (September Camp — one primary focus, not 6)

> **Full schedule**: `DAILY-BLUEPRINT.md` (September Camp edition — print this)
> **Reels workflow**: Hermes skill `gemini-omni-reel` (13-shot list, automated editing)

| Block | When | Focus |
|-------|------|-------|
| **Golden Block** | ~05:15-06:00 | DSA — revision 20 min + 1 new problem (recall first) |
| **Block 2** | 06:00-06:20 | Spring Boot theory (20 min) |
| **Muay Thai Camp** | 07:00-08:30 | BRUTE Manpada (Mon-Fri) + Sat sparring |
| **Office free time** | 14:00-14:30 | LeetCode / Claude cert (bonus, counts toward bar) |
| **Evening Block 1** | 19:35-20:25 | Spring Boot (50 min) → git commit |
| **Evening Block 2** | 20:35-21:15 | Career work (40 min) — resume/apps/system design |
| **Reset + Clips** | 21:15-21:28 | Physical reset + send clips to Hermes for reel |
| **Journal** | 21:28-21:35 | Analog journal (3 wins, 1 bottleneck, energy, top 3) |
| **Wind-down** | 21:35-22:00 | Haldi doodh, stretch, dhikr, book. NO SCREENS. |
| **Sleep** | 22:00 | Phone OUT of bedroom, greyscale ON |

**Daily total: ~2h50m study (weekday) / 5-6 hrs (Saturday)**
**Late Wake Protocol:** never let a late wake kill the evening block. Fajr always first.

---

## 📿 The 3 Make-or-Break Rules

1. **The 5-Min Rule** — home → wudu → rest → *then* anything. First 5 min decide the evening.
2. **Charger out of bedroom** — tahajjud dies at 2 AM scrolling, not at the 04:45 alarm.
3. **22:00 sleep** — wind-down at 21:35, no screens. 7h10m is your foundation.

---

## 📋 Weekly Review (every Friday)

1. What clicked? What didn't?
2. DSA count? Help-flag trend (🟢/🟡/🟠)?
3. Sleep by 22:00 — how many nights?
4. Fajr — how many days? Tahajjud?
5. Reels stole any evenings? How many?
6. Muay Thai camp — energy level?
7. Energy trend (1-5 from journal)?
8. ONE adjustment for next week

**No guilt. No shame. Just data. Adjust and move.**

---

## 🎬 Daily Reels Project (AUTOMATED)

- **You do NOT edit reels. Hermes does.**
- Shoot 13 clips throughout the day → send to Hermes via Discord → Hermes builds the reel.
- Separate Instagram account, documentary style, 20-30 sec/day
- NO prayer footage. NO on-screen stats/prayer counts. Caption-only stats.
- If clips not sent by 21:30 → no reel that day. Carry to tomorrow.
- Full shot list + workflow: Hermes skill `gemini-omni-reel`

---

## 📌 Previous Attempts (NOT counted toward 84)

- V1 (Jul 27-30): 3 DSA, all Copilot
- V2-first (Aug 3-9): 2 working days
- V2-restart (Aug 17): stalled
- V3-restart (Sep 1): **fresh start, carryover cleared**

All arcs complete. Zero guilt. Sep 1 = Day 1.













---

> **Tuesday 04:45. Day 1 of 84. Bismillah.** 🟢
>
> *The gap between you and a ₹16-50 LPA GCC job is not intelligence. It's 84 days of showing up.*
>
> *Missed days extend the arc. They never reset it. Despair is the enemy — never you.*
