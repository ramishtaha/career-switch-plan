# 📋 Progress Tracker — Ramish's 12-Week Plan V4-restart

> Auto-updated by Hermes via Evening Close-Out. Read at session start.
> COMPLETION-LOCKED: Day N = Nth day WORKED, not calendar date. Missed days extend arc, never reset.

---

## Overall Stats

> **Counts live in `session-state.md` — this file is the GRID only.**
> If counts disagree, `session-state.md` wins. See `tracker/state-machine.md`.

- **Plan Version:** V4-restart (fresh start, Sep 15, 2026)
- **Active Days Worked:** 0 / 84 *(see session-state.md for current)*
- **DSA / Designs / Mocks / Apps:** *(see session-state.md — single source of truth)*
- **Career Path:** Plan A (Java+Spring→BFSI GCC) primary, B (AI) fallback Wk6, C (delay) safety net
- **System Phase:** `active` — V4 restart (Day 1 = Tue Sep 15, 2026; all trackers reset per Ramish's request)
- **Pre-Restart:** V1 (Jul 27-30, 3 DSA Copilot), V2-first (Aug 3-9, 2 days), V2-restart (Aug 17, stalled), V3-restart (Sep 1, died Sep 4 — 9 missed days). Fresh start Sep 15, 2026 (V4). All carryover cleared.

---

## Daily Bar System

| Bar | Requirement |
|-----|-------------|
| 🟩 **FLOOR** (streak alive) | Fajr + camp (or rest) + current salah rung → **streak PRESERVED, Day does NOT increment** |
| 🔵 **DAY** (a real day) | Floor + one worked item (1 DSA / 30-min Spring Boot / design / mock) → Day +1 |
| 🟡 **STRONG** (push day) | Floor + 2+ DSA + Spring Boot commit → Day +1 |
| 😴 **Rest Day** | "rest day" → zero guilt, arc continues |
| ⬜ **Missed** | Floor not met → streak resets, arc extends. One sentence. Resume. |

---

## PHASE 1: FOUNDATION (Weeks 1-4) — "Build the Machine"

### WEEK 1 (Sep 15-21): DSA Basics + Spring Boot Skeleton + Habit System
**Theme:** Get the machine running. NO cloud. NO Kubernetes. NO AI. Just basics.
**Primary Focus:** Arrays & Hashing (DSA) + Spring Boot Project Setup
**Week 1 REAL Goal:** Habit system locked in (phone out of bedroom, greyscale, evening anchor)

| # | Day | Date | DSA | Spring Boot | Habit System | Bar |
|---|-----|------|-----|-------------|-------------|-----|
| 1 | Tue | Sep 15 | Contains Duplicate, Two Sum (UNAIDED re-solve) | Setup JDK 17, IntelliJ, Maven, start.spring.io, push to GitHub | Charger OUT, greyscale ON, app blocker | Day 1 ✅ |
| 2 | Wed | Sep 16 | Valid Anagram, Group Anagrams (UNAIDED) | Product entity + Repository, H2 DB, GET/POST endpoints | Sleep 22:00, Fajr on time | Day 2 ✅ |
| 3 | Thu | Sep 17 | Top K Frequent, Encode/Decode Strings | ProductService layer, @ControllerAdvice exception handling | 5-Min Rule (home→wudu→pray) | Day 3 ✅ (last of first stint) |
| 4 | Fri | Sep 18 | Products of Array Except Self | Category entity (One-to-Many), validation | Qailulah 25-min timer | ⬜ missed |
| 5 | Sat | Sep 19 | Revision: re-solve 2 without notes | Clean code, comments, 2 unit tests | Weekly Review | ⬜ missed |
| 6 | Sun | Sep 20 | Valid Palindrome, Two Sum II | Validation (@Valid), Postman testing | Batch cook Sunday | ⬜ missed |
| 7 | Mon | Sep 21 | Revision + REST | REST | Journal: weekly prep for Monday | ⬜ missed |

**Week 1 Deliverables:**
- [ ] 8 DSA problems (Arrays, Hashing, Two Pointers start)
- [ ] Spring Boot CRUD app running locally with H2
- [ ] Project on GitHub with 5+ commits
- [ ] Phone out of bedroom for 7 consecutive nights
- [ ] Sleep before 22:00 at least 5 nights

---

### WEEK 2 (Oct 7-28): Sliding Window + Docker + Resume v1
**Theme:** DSA patterns deepen. Containerize. Start resume.
**Primary Focus:** DSA patterns + Docker + Resume

| # | Day | DSA | Spring Boot / DevOps | Career | Bar |
|---|------|-----|---------------------|--------|-----|
| 8 | Tue | Buy/Sell Stock, Longest Substring No Repeat | Dockerize app, Dockerfile | Resume v1 brain dump | ⬜ |
| 9 | Wed | Longest Repeating Char Replace, Permutation in String | Docker Compose: app + PostgreSQL | Resume structure | ⬜ |
| 10 | Thu | Minimum Window Substring, Fruit Into Baskets | Spring Security JWT | TCS experience bullets | ⬜ |
| 11 | Fri | Binary Search, Search 2D Matrix | Spring Actuator + metrics | Skills section | ⬜ |
| 12 | Sat | Koko Eating Bananas, Search Rotated Array | Spring Profiles (dev/prod) | Resume v1 full draft | ⬜ |
| 13 | Sun | Time-Based KV Store, Median Two Sorted Arrays | Spring Boot Test (@MockBean) | Share resume with peers | ⬜ |
| 14 | Mon | Revision (3 no notes) | Refactor + clean code | Resume v1 sent | ⬜ |

**Week 2 Deliverables:**
- [ ] 8 DSA problems (Sliding Window, Binary Search)
- [ ] App Dockerized with PostgreSQL
- [ ] Spring Security, Actuator, Profiles, Tests
- [ ] Resume v1 complete + sent to peers

---

### WEEK 3 (Sep 15-Oct 5): Trees + First System Design + Start Applications
**Theme:** Add distributed systems thinking. Start the referral engine.
**Primary Focus:** Trees (DSA) + System Design intro + Microservices

| # | Day | DSA | Spring Boot | System Design | Career | Bar |
|---|------|-----|-------------|---------------|--------|-----|
| 15 | Tue | Invert BT, Max Depth, Diameter | Split into 2 microservices, RestTemplate | Read Grokking intro | Resume v2 | ⬜ |
| 16 | Wed | Balanced BT, Same Tree, Subtree | API Gateway (Cloud Gateway) | URL Shortener (capacity, base62) | LinkedIn optimization | ⬜ |
| 17 | Thu | BST Search/Insert/Delete | Eureka Service Discovery | URL Shortener (draw + explain) | Connect 10 BFSI recruiters | ⬜ |
| 18 | Fri | Kth Smallest BST, Validate BST | Resilience4j Circuit Breaker | Read: Kafka vs RabbitMQ | Start referral outreach | ⬜ |
| 19 | Sat | Construct BT, Level Order | Refactor + clean code | Read: Load balancers, caching | Apply to 5 roles | ⬜ |
| 20 | Sun | BT Right Side View, Count Good Nodes | First LLM API call (cURL to GLM) | Rate Limiter (token bucket) | Connect 10 more recruiters | ⬜ |
| 21 | Mon | Revision (3 tree problems) | Revision | Rate Limiter (draw + explain) | Resume v2 + LinkedIn done | ⬜ |

**Week 3 Deliverables:**
- [ ] 8 DSA problems (Trees, BST)
- [ ] 2-microservice project with Gateway + Eureka + Circuit Breaker
- [ ] First LLM API call working
- [ ] 2 system designs started (URL Shortener, Rate Limiter)
- [ ] Resume v2 + LinkedIn optimized
- [ ] 10 recruiter connections + 5 applications started

---

### WEEK 4 (Oct 6-12): Heaps + Kafka + Cloud Deploy
**Theme:** Add the missing distributed systems skill. Deploy to cloud.
**Primary Focus:** Heaps (DSA) + Kafka integration

| # | Day | DSA | Spring Boot / DevOps | System Design | Career | Bar |
|---|------|-----|---------------------|---------------|--------|-----|
| 22 | Tue | Kth Largest in Stream, Last Stone Weight | Kafka: Docker local, producer (Product Service) | Web Crawler (BFS, dedup) | Follow up referrals | ⬜ |
| 23 | Wed | K Closest Points, Task Scheduler | Kafka consumer (Category Service) | Web Crawler (explain) | Connect 10 more | ⬜ |
| 24 | Thu | Merge K Sorted Lists, Find Median | Kafka consumer groups, partitions | Notification System | Apply to 5 roles | ⬜ |
| 25 | Fri | Heaps revision | Deploy to DigitalOcean Droplet | Notification System (explain) | Apply to 5 more | ⬜ |
| 26 | Sat | Re-solve 3 heaps no notes | GitHub Actions CI pipeline | Review designs | Application tracker review | ⬜ |
| 27 | Sun | Re-solve 3 more | Verify app on public IP | — | Connect 5 more | ⬜ |
| 28 | Mon | Revision | Kafka integration review | — | Application tracker review | ⬜ |

**Week 4 Deliverables:**
- [ ] 8 DSA problems (Heaps)
- [ ] Kafka producer + consumer in project
- [ ] App deployed to DigitalOcean
- [ ] GitHub Actions CI
- [ ] 2 system designs (Web Crawler, Notification System)
- [ ] 10+ applications submitted
- [ ] ~34 DSA total (on track for ~100 by Week 12)

---

## PHASE 2: DEPTH (Weeks 5-8) — "Interview-Grade Engineer"

### WEEK 5 (Oct 13-19): Graphs + Spring Internals + AI Integration
**Theme:** Spring Boot under the hood. Add the AI differentiator.

| # | Day | DSA | Spring Boot | AI Integration | Career | Bar |
|---|------|-----|-------------|----------------|--------|-----|
| 29 | Tue | Number of Islands, Max Area | Auto-configuration deep dive | LangChain4j dependency | Mock 1 (pramp DSA) | ⬜ |
| 30 | Wed | Clone Graph, Course Schedule | Bean lifecycle | Chat memory feature | Apply to 5 | ⬜ |
| 31 | Thu | Pacific Atlantic, Course Schedule II | AOP (@Before, @Around, logging aspect) | /ai/generate-description endpoint | Mock 2 (pramp DSA) | ⬜ |
| 32 | Fri | Connected Components, Graph Valid Tree | Conditional beans | — | Apply to 5 | ⬜ |
| 33 | Sat | Redundant Connection, Accounts Merge | Spring Boot revision | — | Connect 5 recruiters | ⬜ |
| 34 | Sun | 5 graph problems re-solve | Spring Boot cheat sheet | LangChain4j doc loading | Mock review | ⬜ |
| 35 | Mon | Revision | Revision | — | Application tracker | ⬜ |

**Week 5 Deliverables:**
- [ ] 8 DSA problems (Graphs, Union Find)
- [ ] Spring Boot internals (auto-config, bean lifecycle, AOP, conditional beans)
- [ ] LangChain4j integrated with chat memory + /ai endpoint
- [ ] 2 mock interviews done
- [ ] 20+ applications submitted

---

### WEEK 6 (Oct 20-26): Tries + Concurrency + RAG + ⚠️ DECISION POINT
**Theme:** Java concurrency is a core interview topic. Master it.
**⚠️ END OF WEEK 6 — PLAN A/B/C DECISION:**
- Spring Boot clicking → continue Plan A
- AI thriving but Spring struggles → consider Plan B
- Both struggling → simplify, extend timeline, Plan C

| # | Day | DSA | Spring Boot | AI Integration | System Design | Career | Bar |
|---|------|-----|-------------|----------------|---------------|--------|-----|
| 36 | Tue | Implement Trie, Search Suggestions | Threads, thread pools | pgvector extension | Transaction Processing (BANKING!) | Mock 3 | ⬜ |
| 37 | Wed | Word Search II | synchronized, volatile, Locks | Store embeddings | Transaction Processing (draw) | Mock review | ⬜ |
| 38 | Thu | Subsets, Combination Sum | CompletableFuture | RAG endpoint (retrieve→LLM→answer) | Transaction Processing (explain) | Apply to 5 | ⬜ |
| 39 | Fri | Permutations, Word Search | Virtual Threads (Java 21) | Semantic search ("similar to X") | Event sourcing + CQRS | 5 referrals | ⬜ |
| 40 | Sat | N-Queens, Sudoku Solver | ExecutorService vs ForkJoinPool | — | Transaction Processing final | Mock 4 (SD 45min) | ⬜ |
| 41 | Sun | 5 problems re-solve | Concurrency cheat sheet | — | — | Review all 4 mocks | ⬜ |
| 42 | Mon | Revision | Revision | — | — | Half-point review + ⚠️ DECISION | ⬜ |

**Week 6 Deliverables:**
- [ ] 8 DSA problems (Tries, Backtracking)
- [ ] Java concurrency mastered (threads, locks, CompletableFuture, virtual threads)
- [ ] RAG endpoint with pgvector (semantic search + retrieval augmented generation)
- [ ] Transaction Processing System design (banking domain!)
- [ ] 4 mock interviews done
- [ ] Claude cert prep started (if TCS offers free)

---

### WEEK 7 (Oct 27-Nov 2): DP 1D + JVM Internals + Observability
**Theme:** DP is the hardest DSA topic. Start early. JVM = senior engineer differentiator.

| # | Day | DSA | Spring Boot / DevOps | System Design | Career | Bar |
|---|------|-----|---------------------|---------------|--------|-----|
| 43 | Tue | Climbing Stairs, Min Cost, House Robber | JVM memory model (heap, stack, metaspace) | Market Data Streaming (BANKING!) | Mock 5 | ⬜ |
| 44 | Wed | House Robber II, Longest Palindromic Substring | JVM GC (G1GC, ZGC, Serial) | Market Data (draw) | Mock review | ⬜ |
| 45 | Thu | Word Break, Coin Change | JVM classloading | Market Data (explain) | Apply to 5 | ⬜ |
| 46 | Fri | LIS, Buy/Sell Cooldown | JVM memory leaks, heap dump | Time-series DBs | Mock 6 (SD) | ⬜ |
| 47 | Sat | LCS, Decode Ways | JVM thread dump, jstack | Market Data final | Mock review | ⬜ |
| 48 | Sun | 5 DP problems re-solve | JVM cheat sheet | Review all designs | Connect 5 recruiters | ⬜ |
| 49 | Mon | Revision | Revision | — | Application tracker | ⬜ |

**Week 7 Deliverables:**
- [ ] 8 DSA problems (DP 1D)
- [ ] JVM internals (memory, GC, classloading, diagnostics)
- [ ] Market Data Streaming Platform design (banking domain!)
- [ ] Security (OWASP, JWT, OAuth2)
- [ ] 6 mock interviews done

---

### WEEK 8 (Nov 3-9): DP 2D + Microservices Patterns + Mid-Point Review
**Theme:** Advanced DP + production patterns. Halfway checkpoint.

| # | Day | DSA | Spring Boot | AI Integration | System Design | Career | Bar |
|---|------|-----|-------------|----------------|---------------|--------|-----|
| 50 | Tue | Unique Paths, Triangle | CQRS pattern | RAG caching (Spring Cache + Redis) | Regulatory Reporting (BANKING!) | Mock 7 | ⬜ |
| 51 | Wed | Longest Palindromic Subseq, Interleaving String | Saga pattern | Streaming response (SSE) | Regulatory Reporting (draw) | Mock review | ⬜ |
| 52 | Thu | Edit Distance, Distinct Subsequences | Outbox pattern | Input validation + rate limiting | Regulatory Reporting (explain) | Apply to 5 | ⬜ |
| 53 | Fri | Burst Balloons, Regex Matching | API composition, BFF | — | Data partitioning | Mock 8 (SD) | ⬜ |
| 54 | Sat | Stone Game, Minimum Path Sum | Microservices revision | RAG final polish | Regulatory Reporting final | Review all 8 mocks | ⬜ |
| 55 | Sun | 5 DP 2D re-solve | Microservices cheat sheet | AI architecture README | Review all 8 designs | Application tracker | ⬜ |
| 56 | Mon | Revision | Revision | — | — | Half-point review | ⬜ |

**Week 8 Deliverables:**
- [ ] 8 DSA problems (DP 2D)
- [ ] Microservices patterns (CQRS, Saga, Outbox, API composition)
- [ ] Regulatory Reporting Pipeline design (your domain!)
- [ ] RAG endpoint with caching, streaming, validation, rate limiting
- [ ] 8 mock interviews done
- [ ] ~66 DSA total

---

## PHASE 3: SHARPENING (Weeks 9-12) — "Interview Ready"

### WEEK 9 (Nov 10-16): LeetCode Speed Run + Security + Active Interviews
**Theme:** DSA speed. Security hardening. Start active interviewing.

| # | Day | DSA | Spring Boot | DevOps | Career | Bar |
|---|------|-----|-------------|--------|--------|-----|
| 57 | Tue | 5 mediums (20-min timed) | OWASP Top 10 | CI/CD multi-env (dev→staging→prod) | Resume final polish | ⬜ |
| 58 | Wed | 5 mediums (timed) | JWT deep dive + refresh rotation | GitHub Actions CD (auto-deploy K8s) | Mock 9 (DSA speed) | ⬜ |
| 59 | Thu | 5 mediums (timed) | OAuth2 (Google/GitHub login) | Test CD pipeline | **START ACTIVE INTERVIEWING** | ⬜ |
| 60 | Fri | 5 mediums (timed) | Spring Security revision | — | Active interviews | ⬜ |
| 61 | Sat | 5 mediums (timed) | — | — | Mock 10 (SD) | ⬜ |
| 62 | Sun | 10 re-solve (no notes, timed) | Spring Boot + Java cheat sheets | — | Connect 5 recruiters | ⬜ |
| 63 | Mon | Revision | Revision | — | Application tracker | ⬜ |

**Week 9 Deliverables:**
- [ ] 25+ LeetCode mediums under timed conditions
- [ ] **ALL Copilot-flagged problems re-solved unaided** (non-negotiable)
- [ ] Security (OWASP, JWT, OAuth2)
- [ ] Multi-env CI/CD pipeline
- [ ] Resume final version
- [ ] 10 mock interviews done
- [ ] Active interviewing started

---

### WEEK 10 (Nov 17-23): LeetCode Hard + System Design Mocks
**Theme:** Hard problems. System design under pressure.

| # | Day | DSA | System Design | Career | Bar |
|---|------|-----|---------------|--------|-----|
| 64 | Tue | Median Two Sorted Arrays, Trapping Rain Water | Risk Limits Enforcement (BANKING!) | Active interviews | ⬜ |
| 65 | Wed | Largest Rect Histogram, Merge K Lists | Risk Limits (explain) | Active interviews | ⬜ |
| 66 | Thu | Word Ladder, Basic Calculator | Regulatory Reporting Pipeline (BANKING!) | Active interviews | ⬜ |
| 67 | Fri | Sliding Window Max, Longest Valid Parens | Regulatory Reporting (explain) | Active interviews | ⬜ |
| 68 | Sat | N-Queens, Sudoku Solver, Jump Game II | Mock system design (random prompt) | Active interviews | ⬜ |
| 69 | Sun | 5 hard re-solve | Mock system design (random prompt) | Follow up interviews | ⬜ |
| 70 | Mon | Revision | Review all designs | Application tracker | ⬜ |

**Week 10 Deliverables:**
- [ ] 15+ LeetCode hards solved
- [ ] 2 banking-domain system designs (Risk Limits, Regulatory Reporting)
- [ ] Active interviews in progress
- [ ] 12 mock interviews done

---

### WEEK 11 (Nov 24-30): Contest Simulation + Final Project Polish
**Theme:** Contest-level DSA. Project documentation.

| # | Day | DSA | Project | System Design | Career | Bar |
|---|------|-----|---------|---------------|--------|-----|
| 71 | Tue | LeetCode contest (4 problems, 90 min) | Final production deploy (2 services + Postgres + Gateway + Kafka + RAG) | Mock: distributed cache (45 min) | Active interviews | ⬜ |
| 72 | Wed | LeetCode contest (4 problems, 90 min) | README: architecture diagram + data flow + security | Mock: distributed message queue | Active interviews | ⬜ |
| 73 | Thu | Contest problems re-solve | Document AI feature (RAG architecture) | Mock: transaction processing | Active interviews | ⬜ |
| 74 | Fri | 5 hards (25-min each) | Project demo prep (5 min) | Mock: rate limiter | Active interviews | ⬜ |
| 75 | Sat | 5 hards (25-min each) | — | Mock: market data streaming | Active interviews | ⬜ |
| 76 | Sun | 10 re-solve (no notes) | Final project review | Review all mock designs | Review interviews | ⬜ |
| 77 | Mon | Revision | — | Review | Application tracker | ⬜ |

**Week 11 Deliverables:**
- [ ] 20+ LeetCode hards + contest simulation
- [ ] System design mock practice (4+ full mocks)
- [ ] Project fully deployed + documented
- [ ] Active interviews in progress
- [ ] Confidence in explaining Spring Boot, Java, system design aloud

---

### WEEK 12 (Dec 1-7): Final Revision + Offers + Negotiation
**Theme:** Close the loop. Get the offer.

| # | Day | DSA | Spring Boot | System Design | Career | Bar |
|---|------|-----|-------------|---------------|--------|-----|
| 78 | Tue | 10 problems (timed, no notes) | All cheat sheets review | Strongest design (15 min, record) | Active interviews | ⬜ |
| 79 | Wed | 2 hrs weakest pattern | Project demo prep (5 min) | 2nd strongest (15 min) | Active interviews | ⬜ |
| 80 | Thu | 2 hrs weakest pattern | Mock: project + Spring Q&A (30 min) | 3rd strongest (15 min) | Negotiation prep | ⬜ |
| 81 | Fri | 5 mixed (timed) | Final cheat sheet review | Random prompt (30 min) | Active interviews | ⬜ |
| 82 | Sat | 5 mixed (timed) | Final project review | Final cheat sheet review | Active interviews + negotiation | ⬜ |
| 83 | Sun | 10 problems (final practice) | Final demo run-through | Final system design run | Offer negotiation practice | ⬜ |
| 84 | Mon | Light revision | Light revision | Light revision | Plan next steps + REST | ⬜ |

**Week 12 Deliverables:**
- [ ] ~100 DSA problems solved, 60%+ unaided
- [ ] Production Spring Boot + AI project on GitHub
- [ ] 8 system designs practiced (3 banking-domain)
- [ ] 12 mock interviews done
- [ ] 50+ applications submitted
- [ ] 1+ offer (or actively interviewing into November)
- [ ] If offer received: RESIGN. Start the 90-day clock.

---

## 📝 Notes

- V3 plan starts Tuesday, September 1, 2026
- V1 ran Jul 27-30: 3 DSA problems (all Copilot). V2-first ran Aug 3-9: 2 working days. V2-restart started Aug 17: stalled.
- V3 changes: fresh start Sep 15, all carryover cleared, same V2 curriculum (one primary focus per week, ~100 DSA target, one incremental project)
- Habit system is Week 1's REAL goal — without fixing reels/sleep, nothing else sticks
- Day numbers = working days, NOT calendar dates. Missed days extend the arc, never reset.
- Evening Close-Out cron syncs ALL tracker files when Ramish replies to check-in.
